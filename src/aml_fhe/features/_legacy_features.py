...
def create_spark_graph_dataframe(graph_data, spark):
    graph_data = graph_data.copy(deep=False)
    schema = []
    for column in graph_data.columns:
        if column == "timestamps_amounts":
            graph_data.loc[:, column] = graph_data[column].apply(
                lambda rows: [[float(ts), float(amount)] for ts, amount in rows]
            )
            dtype = st.ArrayType(st.ArrayType(st.DoubleType(), containsNull=False), containsNull=False)
        elif pd.api.types.is_integer_dtype(graph_data[column]):
            dtype = st.LongType()
        elif pd.api.types.is_float_dtype(graph_data[column]):
            dtype = st.DoubleType()
        elif pd.api.types.is_bool_dtype(graph_data[column]):
            dtype = st.BooleanType()
        else:
            dtype = st.StringType()
        schema.append(st.StructField(column, dtype, nullable=True))
    return spark.createDataFrame(graph_data, schema=st.StructType(schema))
...
def generate_features_spark(communities, graph_data, spark, num_cores=os.cpu_count()):
    ...
    graph_data = create_spark_graph_dataframe(graph_data, spark).withColumnRenamed("source", "src").withColumnRenamed("target", "trg")
