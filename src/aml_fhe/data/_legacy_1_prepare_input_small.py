...
with open(s.DATA_FILE, "r") as in_file, open(s.STAGED_DATA_CSV_LOCATION, "w") as out_file:
    for cnt, line in enumerate(in_file):
        ...
        out_file.write(f"{cnt},{line}\n")

        if cnt % 500000 == 0:
            print("staged rows:", cnt, flush=True)
...
with open(s.PATTERNS_FILE, "r") as in_file, open(s.STAGED_PATTERNS_CSV_LOCATION, "w") as out_file:
    for line in in_file:
        line = line.strip()
        ...
        if line[:4].isnumeric():
            ...
            if line_id not in mapping:
                missing += 1
                continue
            out_file.write(f"{mapping[line_id]},{line}\n")
        else:
            out_file.write(f"{line}\n")

print("missing pattern tx:", missing)
