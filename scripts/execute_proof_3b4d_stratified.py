"""
FHE execute proof — 3-bit depth-4 50-tree (recommended setting), stratified sample.

Sample: 300 illicit + 2700 normal = 3000 rows (10% illicit, ~50x natural IR).
Proves FHE execute==clear match on a large, distribution-representative sample.

Outputs:
  artifacts/execute_proof/execute_proof_3b4d_stratified.json
  artifacts/execute_proof/execute_proof_3b4d_stratified.log
"""
...
OUTPUT_JSON    = OUTPUT_DIR / "execute_proof_3b4d_stratified.json"
OUTPUT_LOG     = OUTPUT_DIR / "execute_proof_3b4d_stratified.log"
N_ILLICIT_ROWS = 300
N_NORMAL_ROWS  = 2700
RANDOM_SEED    = 42
...
def setup_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("execute_proof_3b4d_strat")
...
def main() -> None:
    ...
    logger.info("=== FHE Execute Proof — 3b4d50t stratified (%s illicit + %s normal) ===",
                N_ILLICIT_ROWS, N_NORMAL_ROWS)
    ...
    logger.info("Circuit: complexity=%.3e  bootstrap_keys=%s MB  mlir_chars=%s",
                circuit_info["complexity"],
                round(circuit_info["size_of_bootstrap_keys"] / 1e6, 1),
                circuit_info["mlir_chars"])

    # Stratified sample: N_ILLICIT_ROWS illicit + N_NORMAL_ROWS normal
    ...
    output = {
        "experiment":        "execute_proof_3b4d_stratified",
