"""
FHE execute proof — 3-bit depth-5 50-tree seed=1 (recommended setting).

Sample: 300 illicit + 2700 normal = 3000 rows.
Proves FHE execute==clear match on the actual recommended config.

Outputs:
  artifacts/execute_proof/execute_proof_3b5d_s1.json
  artifacts/execute_proof/execute_proof_3b5d_s1.log
"""
...
OUTPUT_JSON    = OUTPUT_DIR / "execute_proof_3b5d_s1.json"
OUTPUT_LOG     = OUTPUT_DIR / "execute_proof_3b5d_s1.log"
N_ILLICIT_ROWS = 300
N_NORMAL_ROWS  = 2700
...
def setup_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("execute_proof_3b5d_s1")
...
def main() -> None:
    ...
    logger.info("=== FHE Execute Proof — 3b5d50t seed=1 (RECOMMENDED) ===")
    logger.info("Sample: %s illicit + %s normal = %s rows", N_ILLICIT_ROWS, N_NORMAL_ROWS, N_ILLICIT_ROWS + N_NORMAL_ROWS)
    logger.info("Config: %s", CONFIG_PATH)

    config = load_pipeline_config(CONFIG_PATH)
    seed = config.model.random_seed  # 1
    ...
    logger.info("Circuit: complexity=%.3e  bootstrap_keys=%.1f MB  mlir_chars=%s",
                circuit_info["complexity"],
                circuit_info["size_of_bootstrap_keys"] / 1e6,
    ...
    logger.info("Test loaded in %.1fs  shape=%s  illicit=%s (%.4f%%)",
                time.time() - t0, x_test.shape, f"{int(y_test.sum()):,}", y_test.mean() * 100)

    illicit_idx = np.where(y_test == 1)[0]
    normal_idx  = np.where(y_test == 0)[0]
    rng = np.random.default_rng(seed)
    illicit_selected = rng.choice(illicit_idx, size=min(N_ILLICIT_ROWS, len(illicit_idx)), replace=False)
    normal_selected  = rng.choice(normal_idx,  size=N_NORMAL_ROWS, replace=False)

    selected_idx = np.concatenate([illicit_selected, normal_selected])
    ...
    logger.info("Execute sample: %s rows  (%s illicit = %.1f%%  %s normal)",
                len(x_sample), n_illicit_in_sample,
                n_illicit_in_sample / len(x_sample) * 100, n_normal_in_sample)

    est_seconds = len(x_sample) * 4.0
    logger.info("Estimated execute time: %.1f hours (%.1fs/row est × %s rows)",
                est_seconds / 3600, 4.0, len(x_sample))
    ...
    for i in range(len(x_sample)):
        ...
        if (i + 1) % 100 == 0 or i == 0:
            ...
            logger.info("[%s/%s]  match=%s/%s (%.1f%%)  avg=%.2fs/row  remaining≈%.1fh",
                        i + 1, len(x_sample),
                        matches_so_far, i + 1,
    ...
    output = {
        "experiment":        "execute_proof_3b5d_s1",
        ...
        "sample": {
            ...
            "illicit_pct":          round(n_illicit_in_sample / total_rows * 100, 4),
